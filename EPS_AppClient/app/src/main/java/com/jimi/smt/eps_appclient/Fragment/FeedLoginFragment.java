        ...
        if (!TextUtils.isEmpty(edt_pwd.getText().toString().trim())) {
            ...
            if (pwd.equalsIgnoreCase(Constants.feedPwd)) {
                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        //初始化全部上料、换料、抽检、全检结果
                        int result = new DBService().initAllResult(globalData.getLine(),
                ...
                */
                HttpUtils.getHttpUtils().reset(globalData.getLine(), globalData.getWork_order(), globalData.getBoard_type());
                mActivity.setTabSelection(1);
                edt_pwd.setText("");
            } else {
